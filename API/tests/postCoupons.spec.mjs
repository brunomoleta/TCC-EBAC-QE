...
describe('Dado que você está usando a rota de cupons da API', function() {
    ...
    console.log(apiUrl)
    console.log(token)
    context('Quando é feito POST enviando um corpo de requisição inválido', function() {
        let response
    ...
    context('Quando é feito POST enviando dados válidos com token autenticado', function() {
        let response
        beforeEach(async function() {
            this.timeout(5000);
            response = await request(apiUrl)
